...
// ---------------------------------------------------------------------------
// Animal silhouettes. Filled shape, heavy outline -- the only thing that
// survives being shrunk to a token on a hex.
// ---------------------------------------------------------------------------
function outline(ctx, path, fill, line, lw) {
  ctx.lineJoin = 'round';
  ctx.lineCap = 'round';
...
/** Trace a closed run of points, straight segments unless a radius is given. */
function shape(ctx, pts) {
  ctx.beginPath();
  pts.forEach((p, i) => {
...
/**
 * Each animal is a solid silhouette in its deep colour with one or two
 * lighter accents. Solid beats detail here: these end up 50 pixels across on
 * a tile face, and anything finer turns to mush.
 */
export const ANIMAL_PAINTERS = {
  // Ram's head in profile, the curled horn doing all the identifying.
  bighorn(ctx, R, fill, line) {
    const u = (v) => v * R;
    // The horn first, so the head overlaps its near end.
    ctx.save();
    ctx.lineCap = 'butt';
    ctx.strokeStyle = fill;
    ctx.lineWidth = u(0.3);
    ctx.beginPath();
    ctx.arc(u(0.14), u(-0.04), u(0.46), RAD(140), RAD(60));
    ctx.stroke();
    ctx.strokeStyle = line;
    ...
    shape(ctx, [
      ...
      [u(0.16), u(0.5)],
      [u(-0.22), u(0.64)],
      [u(-0.66), u(0.56), u(-0.95), u(0.3)],
    ]);
    ctx.fill();
    // Ear, tucked behind the horn.
    ctx.beginPath();
    ctx.moveTo(u(0.2), u(-0.2));
    ctx.lineTo(u(0.56), u(-0.34));
    ctx.lineTo(u(0.3), u(0.04));
    ctx.closePath();
    ctx.fill();
    dot(ctx, u(-0.3), u(0.04), u(0.08), fill);
  },

  // Bull elk: long muzzle, six-point rack.
  elk(ctx, R, fill, line) {
    const u = (v) => v * R;
    ctx.save();
    ctx.strokeStyle = line;
    ...
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    const beam = (sx, cx1, cy1, ex, ey, tines) => {
      ctx.beginPath();
      ctx.moveTo(u(sx), u(-0.1));
      ctx.quadraticCurveTo(u(cx1), u(cy1), u(ex), u(ey));
      ctx.stroke();
      for (const [ax, ay, bx, by] of tines) {
        ctx.beginPath();
        ctx.moveTo(u(ax), u(ay));
        ctx.lineTo(u(bx), u(by));
        ctx.stroke();
      }
    };
    beam(-0.12, -0.4, -0.6, -0.16, -0.96, [[-0.29, -0.46, -0.62, -0.58], [-0.24, -0.74, -0.56, -0.88]]);
    beam(0.14, 0.46, -0.54, 0.3, -0.9, [[0.34, -0.42, 0.66, -0.5], [0.32, -0.7, 0.62, -0.84]]);
    ctx.restore();
    ...
    shape(ctx, [
      ...
      [u(0.16), u(0.6)],
      [u(-0.2), u(0.74)],
      [u(-0.66), u(0.72), u(-0.96), u(0.46)],
    ]);
    ctx.fill();
    // Ear.
    ctx.beginPath();
    ctx.ellipse(u(0.24), u(0.06), u(0.2), u(0.11), RAD(-28), 0, Math.PI * 2);
    ctx.fill();
    dot(ctx, u(-0.34), u(0.24), u(0.075), fill);
    // Pale throat patch, the way a bull shows in low sun.
    ctx.save();
    ctx.globalAlpha = 0.75;
    ctx.fillStyle = fill;
    ctx.beginPath();
    ctx.ellipse(u(-0.18), u(0.56), u(0.26), u(0.13), RAD(-12), 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  },

  // Greenback cutthroat: forked tail, high dorsal, a stripe of spots.
  trout(ctx, R, fill, line) {
    const u = (v) => v * R;
    ctx.fillStyle = line;
    // Dorsal and pelvic fins go under the body so they read as attached.
    shape(ctx, [[u(-0.02), u(-0.3)], [u(0.16), u(-0.76)], [u(0.38), u(-0.24)]]);
    ctx.fill();
    shape(ctx, [[u(-0.04), u(0.28)], [u(0.04), u(0.66)], [u(0.3), u(0.3)]]);
    ctx.fill();
    shape(ctx, [
      [u(-0.94), u(0.0)],
      [u(-0.42), u(-0.36), u(0.16), u(-0.34)],
      [u(0.44), u(-0.22)],
      [u(0.94), u(-0.46)],
      [u(0.76), u(0.0)],
      [u(0.94), u(0.46)],
      [u(0.44), u(0.22)],
      [u(0.16), u(0.34), u(-0.42), u(0.36)],
      [u(-0.94), u(0.0)],
    ]);
    ctx.fill();
    // Cutthroat's red slash under the jaw, then the spots.
    ctx.strokeStyle = fill;
    ctx.lineWidth = u(0.08);
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(u(-0.74), u(0.13));
    ctx.lineTo(u(-0.44), u(0.22));
    ctx.stroke();
    for (let i = 0; i < 4; i++) dot(ctx, u(-0.16 + i * 0.2), u(-0.1 + (i % 2) * 0.16), u(0.055), fill);
    dot(ctx, u(-0.62), u(-0.06), u(0.075), fill);
  },

  // Golden eagle on the soar: one flat plank of wing either side.
  eagle(ctx, R, fill, line) {
    ...
    const wing = (s) => {
      shape(ctx, [
        [u(0.04 * s), u(-0.24)],
        [u(0.5 * s), u(-0.52), u(0.98 * s), u(-0.34)],
        [u(0.86 * s), u(-0.04)],
        [u(0.7 * s), u(-0.14)],
        [u(0.6 * s), u(0.08)],
        [u(0.46 * s), u(-0.08)],
        ...
        [u(0.24 * s), u(-0.02)],
        [u(0.14 * s), u(0.2)],
        [u(0.06 * s), u(0.02), u(0.04 * s), u(-0.24)],
      ]);
      ctx.fill();
    };
    wing(-1);
    wing(1);
    // Body, head and fanned tail.
    ctx.beginPath();
    ctx.ellipse(0, u(0.12), u(0.15), u(0.42), 0, 0, Math.PI * 2);
    ctx.fill();
    shape(ctx, [[u(-0.19), u(0.4)], [u(-0.12), u(0.92)], [u(0.12), u(0.92)], [u(0.19), u(0.4)]]);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(0, u(-0.44), u(0.17), 0, Math.PI * 2);
    ctx.fill();
    shape(ctx, [[u(-0.05), u(-0.56)], [u(0.0), u(-0.84)], [u(0.07), u(-0.54)]]);
    ctx.fill();
    // The pale nape that gives the golden eagle its name.
    ctx.save();
    ctx.globalAlpha = 0.8;
    dot(ctx, 0, u(-0.38), u(0.1), fill);
    ctx.restore();
  },

  // Coyote, head thrown back mid-howl.
  coyote(ctx, R, fill, line) {
    const u = (v) => v * R;
    ctx.fillStyle = line;
    shape(ctx, [
      [u(-0.97), u(-0.56)],   // nose, pointed up and away
      [u(-0.6), u(-0.34)],
      [u(-0.28), u(-0.16)],
      [u(-0.2), u(-0.5)],     // near ear
      [u(0.0), u(-0.2)],
      [u(0.22), u(-0.64)],    // far ear
      [u(0.42), u(-0.08)],
      [u(0.64), u(0.34), u(0.46), u(0.8)],
      [u(-0.08), u(0.88)],
      [u(-0.3), u(0.36), u(-0.56), u(0.12)],
      [u(-0.88), u(-0.26)],
      [u(-0.97), u(-0.56)],
    ]);
    ctx.fill();
    dot(ctx, u(-0.2), u(-0.02), u(0.075), fill);
    // Pale muzzle and ruff.
    ctx.save();
    ctx.globalAlpha = 0.72;
    ctx.fillStyle = fill;
    ctx.beginPath();
    ctx.ellipse(u(-0.64), u(-0.3), u(0.24), u(0.1), RAD(-32), 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(u(0.1), u(0.66), u(0.28), u(0.13), RAD(8), 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  },
};
